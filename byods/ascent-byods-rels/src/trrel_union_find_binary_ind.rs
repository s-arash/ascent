...
use ascent::internal::{RelIndexMerge, RelIndexReadAll, RelIndexRead};
...
impl<T: Clone + Hash + Eq> Default for TrRelDelta<T> {
   fn default() -> Self {
      Self { set_connections: Default::default(), rev_set_connections: Default::default(), precursor: Default::default(), total: Default::default() }
   }
}
...
impl<T: Clone + Hash + Eq> TrRelDelta<T> {
   fn ind_0_get(&self, x: &T) -> Option<IteratorFromDyn<&'_ T>> {
      ...
      let res = || sets_of_x.iter().flat_map(|&s| {
         self.total.sets[s].iter()
      });
   ...
   fn ind_0_iter_all<'a>(&'a self) -> IteratorFromDyn<(&'a T, IteratorFromDyn<&'_ T>)> {
      let res = || self.set_connections.iter().flat_map(|(set_id, set_connections)| {
         let xs = &self.total.sets[*set_id];
         xs.iter().map(|x| {
            let ys = || set_connections.iter().flat_map(|sc| {
               self.total.sets[*sc].iter()
            });
   ...
   fn ind_1_iter_all<'a>(&'a self) -> IteratorFromDyn<(&'a T, IteratorFromDyn<&'_ T>)> {
      let res = || self.rev_set_connections.iter().flat_map(|(set_id, rev_set_connections)| {
         let xs = &self.total.sets[*set_id];
         xs.iter().map(|x| {
            let ys = || rev_set_connections.iter().flat_map(|sc| {
               self.total.sets[*sc].iter()
            });
            (x, IteratorFromDyn::new(ys))
         })
   ...
   fn iter_all<'a>(&'a self) -> impl Iterator<Item = (&'a T, &'a T)> + 'a {
      let res = self.set_connections.iter().flat_map(move |(x_set, y_sets)| {
         self.total.sets[*x_set].iter().flat_map(move |x| {
            y_sets.iter().filter(move |y_set| *y_set != x_set).flat_map(move |y_set| {
               self.total.sets[*y_set].iter().map(move |y| (x, y))
            })
         })
      });
      res
...
impl<T: Clone + Hash + Eq> RelIndexMerge for TrRelIndCommon<T> {
   ...
   fn merge_delta_to_total_new_to_delta(new: &mut Self, delta: &mut Self, total: &mut Self) {
      ...
      where Rel1::ValueIteratorType: Clone
      {
      ...
      loop {
         ...
         let join1 = join(&mut delta_new_map, &mut delta_new_rev_map, &MapRelIndexAdaptor(&delta_delta_map), &MapRelIndexAdaptor(&total_rel.reverse_set_connections), &mut can_add, "join1");
         let join2 = join(&mut delta_new_map, &mut delta_new_rev_map, &MapRelIndexAdaptor(&total_rel.set_connections), &MapRelIndexAdaptor(&delta_delta_rev_map), &mut can_add, "join2");
         let join3 = join(&mut delta_new_map, &mut delta_new_rev_map, &MapRelIndexAdaptor(&new_classes_map), &MapRelIndexAdaptor(&delta_delta_rev_map), &mut can_add, "join3");
...
impl<T: Clone + Hash + Eq> ByodsBinRel for TrRelIndCommon<T> {
   ...
   type AllIter<'a> = Box<dyn Iterator<Item = (&'a T, &'a T)> + 'a> where Self: 'a;
   ...
   fn len_estimate(&self) -> usize {
      ...
      match self {
         TrRelIndCommon::Delta { rel, .. } => {
            let avg_set_connections = 
               rel.set_connections.iter().take(3).map(|(_s, sets)| sets.len()).sum::<usize>() / sample_size.min(rel.set_connections.len()).max(1);
   ...
   type Ind0AllIterValsIter<'a> = IteratorFromDyn<'a, &'a T> where Self: 'a;
   type Ind0AllIter<'a> = IteratorFromDyn<'a, (&'a T, Self::Ind0AllIterValsIter<'a>)> where Self: 'a;

   fn ind0_iter_all<'a>(&'a self) -> Self::Ind0AllIter<'a> {
      match self {
         TrRelIndCommon::Delta { rel, .. } => rel.ind_0_iter_all(),
         TrRelIndCommon::Total { rel, .. } => {
            let res = || rel.elem_ids.iter().map(|(x, set_id)| {
   ...
   type Ind0ValsIter<'a> = IteratorFromDyn<'a, &'a T> where Self: 'a;
   ...
   type Ind1AllIterValsIter<'a> = IteratorFromDyn<'a, &'a T> where Self: 'a;
   type Ind1AllIter<'a> = IteratorFromDyn<'a, (&'a T, Self::Ind1AllIterValsIter<'a>)> where Self: 'a;
   ...
   type Ind1ValsIter<'a> = IteratorFromDyn<'a, &'a T> where Self: 'a;
   ...
   fn insert(&mut self, x0: Self::T0, x1: Self::T1) -> bool {
      self.unwrap_new_mut().insert((x0, x1)) 
   }
}
