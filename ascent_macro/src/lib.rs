...
fn ascent_source_impl(input: proc_macro2::TokenStream) -> Result<proc_macro2::TokenStream> {
   ...
   let AscentSourceInput { name, ascent_code, attrs, colon, .. } = syn::parse2(input.into())?;
   ...
   match Parser::parse2(
      ...
      itertools::Either::Right(mut macro_back) => {
         let before_tokens = macro_back.before_tokens;
         macro_back.before_tokens = quote! {
            #(#attrs)*
            #name #colon
            #before_tokens
         };
         // This allows transitive `include_source`s. Disabled for now ...
         // return Ok(macro_back.macro_call_output())
         return Err(syn::Error::new(
            macro_back.include_node.include_source_kw.span, "`ascent_source`s cannot contain `include_source!`",
         ))
      },
   };
...
pub(crate) fn ascent_impl(input: proc_macro2::TokenStream, kind: AscentMacroKind) -> Result<proc_macro2::TokenStream> {
   ...
   let prog = match Parser::parse2(
      ...
      Either::Right(macro_back) => return Ok(macro_back.macro_call_output()),
   };
